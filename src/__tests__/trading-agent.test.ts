...
describe("TradingAgent", () => {
  ...
  beforeEach(() => {
    ...
    mockedEthers.parseUnits = jest
      .fn()
      .mockImplementation((value, decimals = 18) => {
  ...
  describe("getMarkets", () => {
    it("should fetch and return active markets", async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith(
        "https://api.sapience.xyz/markets",
        {
          params: {
            status: "active",
            limit: 50,
  ...
  describe("executeTrade", () => {
    it("should execute a trade", async () => {
      ...
      mockContract.placeTrade.mockResolvedValue({
    ...
    it("should throw an error if trade execution fails", async () => {
      ...
      await expect(
        agent.executeTrade(mockPrediction, mockMarket),
  ...
  describe("evaluateTrade", () => {
    ...
    it("should throw error if Groq API fails", async () => {
      ...
      await expect(
        agent.evaluateTrade(mockMarket, mockForecast),
    ...
    it("should throw error if response is not valid JSON", async () => {
      ...
      await expect(
        agent.evaluateTrade(mockMarket, mockForecast),
      ).rejects.toThrow("Could not extract JSON from model response");
    });
